"""Lightweight contracts for ABMIL training on Virchow2 embeddings."""
...
def make_config(embeddings: Path, output_root: Path) -> dict:
    return {
        "experiment_name": "abmil_virchow2_binary",
        ...
        "entregables_dir": str(output_root.parent / "entregables" / "abmil_virchow2_binary_resultados"),
        "seed": 42,
        "device": "cpu",
        "epochs": 1,
        "batch_size_bags": 1,
...
class Virchow2DatasetContractTests(unittest.TestCase):
    def test_accepts_valid_virchow2_artifact(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertEqual(sample["features"].shape, (3, 1280))
    ...
    def test_rejects_wrong_embedding_dim(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            write_fake_virchow2_embedding(
                root / "train" / "slide-wrong-dim.pt",
                embedding_dim=1536,
            )

            dataset = Virchow2EmbeddingDataset(root, split="train")
            with self.assertRaisesRegex(Virchow2DatasetError, "embedding_dim"):
                _ = dataset[0]
    ...
    def test_rejects_wrong_encoder_family(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            write_fake_virchow2_embedding(
                root / "train" / "slide-wrong-family.pt",
                encoder_family="UNI2-h",
            )

            dataset = Virchow2EmbeddingDataset(root, split="train")
            with self.assertRaisesRegex(Virchow2DatasetError, "encoder_family"):
                _ = dataset[0]

    def test_collate_handles_variable_bags(self) -> None:
        batch = [
            {
        ...
        collated = virchow2_bag_collate_fn(batch)

        self.assertEqual(len(collated["features"]), 2)
        self.assertEqual(tuple(collated["labels"].shape), (2,))


class ABMILVirchow2ConfigAndScriptTests(unittest.TestCase):
    def test_expected_files_exist(self) -> None:
        expected_files = [
            PROJECT_ROOT / "configs" / "abmil_virchow2_train_binary.yaml",
            ...
            PROJECT_ROOT / "docs" / "experiments" / "abmil_virchow2_training.md",
        ]
    ...
    def test_scripts_compile(self) -> None:
        for script_name in (
            "21_train_abmil_virchow2_binary.py",
            "22_evaluate_abmil_virchow2_binary.py",
        ):
            py_compile.compile(
    ...
    def test_config_points_to_virchow2_embeddings_and_output(self) -> None:
        config_path = PROJECT_ROOT / "configs" / "abmil_virchow2_train_binary.yaml"
        ...
        self.assertEqual(
            config["output_root"],
            "/content/drive/MyDrive/PANDA_PROSTATE/outputs/abmil_virchow2_binary",
        )
    ...
    def test_official_abmil_uni2h_files_still_exist(self) -> None:
        self.assertTrue(
            (PROJECT_ROOT / "scripts" / "11_train_abmil_uni2h_binary.py").is_file()
        )
        self.assertTrue(
            (PROJECT_ROOT / "scripts" / "12_evaluate_abmil_uni2h_binary.py").is_file()
        )
        self.assertTrue((PROJECT_ROOT / "src" / "mil" / "abmil.py").is_file())
        self.assertTrue((PROJECT_ROOT / "src" / "mil" / "uni2h_dataset.py").is_file())

    def test_train_help_works(self) -> None:
        result = subprocess.run(
            [
                sys.executable,
                str(PROJECT_ROOT / "scripts" / "21_train_abmil_virchow2_binary.py"),
                "--help",
            ],
            cwd=PROJECT_ROOT,
            capture_output=True,
    ...
    def test_evaluate_help_works(self) -> None:
        result = subprocess.run(
            [
                sys.executable,
                str(PROJECT_ROOT / "scripts" / "22_evaluate_abmil_virchow2_binary.py"),
                "--help",
            ],
            cwd=PROJECT_ROOT,
            capture_output=True,
    ...
    def test_abmil_forward_accepts_virchow2_dim(self) -> None:
        model = ABMIL(input_dim=1280, hidden_dim=8, attention_dim=4, dropout=0.0)
        logit, attention = model(torch.randn(5, 1280))

        self.assertEqual(tuple(logit.shape), ())
        self.assertEqual(tuple(attention.shape), (5,))


class ABMILVirchow2ScriptIntegrationTests(unittest.TestCase):
    def test_dry_run_builds_model_without_writing_checkpoints(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertEqual(result.returncode, 0, msg=result.stdout + result.stderr)
            self.assertIn("Dry-run ABMIL + Virchow2 completado", result.stdout)
    ...
    def test_one_epoch_training_and_final_evaluation_on_cpu(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            entregable = output_root.parent / "entregables" / "abmil_virchow2_binary_resultados"
            self.assertTrue((entregable / "resumen_resultados_abmil_virchow2.json").is_file())
