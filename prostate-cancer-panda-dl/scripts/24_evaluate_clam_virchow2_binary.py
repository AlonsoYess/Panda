"""Final validation and test evaluation for CLAM + Virchow2."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Evalua best_model.pt de CLAM + Virchow2 en valid y test."
    )
    parser.add_argument(
        "--config",
        type=Path,
...
def evaluate_clam(
    ...
    if not losses:
        raise RuntimeError("El DataLoader de evaluacion CLAM + Virchow2 no produjo batches.")
...
def create_deliverables(
    *,
    ...
    deliverable_dir = Path(
        config.get(
            "entregables_dir",
            output_root.parent.parent / "entregables" / "clam_virchow2_binary_resultados",
        )
    )
    ...
    save_json(summary, deliverable_dir / "resumen_resultados_clam_virchow2.json")
...
def run(args: argparse.Namespace) -> int:
    ...
    save_confusion_matrix(
        ...
        f"CLAM + Virchow2 Valid Confusion Matrix (Youden={threshold_youden:.4f})",
    )
    save_confusion_matrix(
        test_youden_metrics["confusion_matrix"],
        plots_dir / "confusion_matrix_test.png",
        f"CLAM + Virchow2 Test Confusion Matrix (Valid Youden={threshold_youden:.4f})",
    )
    save_roc_curve(
        valid_result["labels"],
        valid_result["probabilities"],
        plots_dir / "roc_valid.png",
        "CLAM + Virchow2 Validation ROC",
    )
    save_roc_curve(
        test_result["labels"],
        test_result["probabilities"],
        plots_dir / "roc_test.png",
        "CLAM + Virchow2 Test ROC",
    )
