"""Train resumable ABMIL on precomputed UNI2-h embeddings."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Entrena ABMIL binario con embeddings UNI2-h 1536-D."
    )
    parser.add_argument(
        "--config",
        type=Path,
...
def validate_config(config: Dict[str, Any]) -> None:
    ...
    if int(config["input_dim"]) != EXPECTED_EMBEDDING_DIM:
        raise ValueError("input_dim debe ser 1536 para UNI2-h.")
...
def run(args: argparse.Namespace) -> int:
    ...
    if args.dry_run:
        ...
        print("[INFO] Dry-run completado; no se entreno ni se modificaron checkpoints.")
        print(f"[INFO] Sample slide_id: {sample['slide_id']}")
        print(f"[INFO] Sample features: {tuple(sample['features'].shape)}")
    ...
    for epoch in range(start_epoch, final_epoch + 1):
        ...
        score, monitor_used = monitor_score(
            valid_result,
        ...
        save_loss_history(history, plots_dir / "training_loss.png")
        save_metric_history(history, plots_dir / "validation_auc_f1.png")
    ...
    print(f"[INFO] Entrenamiento finalizado. Best epoch: {best_epoch}")
