"""Extract Prov-GigaPath embeddings from advanced PANDA tile ZIPs.
...
tiles with Prov-GigaPath, and writes one .pt embedding artifact per WSI.
"""
...
from src.encoders.provgigapath import (
    EXPECTED_PROVGIGAPATH_DIM,
    PROVGIGAPATH_MODEL_ID,
    ProvGigaPathEncoder,
    validate_embedding_tensor as validate_provgigapath_tensor,
)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Extract Prov-GigaPath embeddings from advanced tile ZIPs."
    )
    parser.add_argument("--config", type=Path, required=True)
    parser.add_argument("--zips-dir", type=Path, required=True)
    parser.add_argument("--output-root", type=Path, required=True)
    parser.add_argument("--encoder", choices=("provgigapath",), default=None)
...
def validate_config(config: Dict[str, Any]) -> None:
    ...
    if str(config["encoder"]) != "provgigapath":
        raise ValueError("encoder debe ser 'provgigapath'.")
    if str(config["model_name"]) != PROVGIGAPATH_MODEL_ID:
        raise ValueError(f"model_name debe ser '{PROVGIGAPATH_MODEL_ID}'.")
    if int(config["embedding_dim"]) != EXPECTED_PROVGIGAPATH_DIM:
        raise ValueError(f"embedding_dim esperado: {EXPECTED_PROVGIGAPATH_DIM}.")
...
def encode_image_tensors(
    ...
    *,
    ...
    expected_dim = int(config["embedding_dim"])
    validate_provgigapath_tensor(features, expected_dim=expected_dim)
...
def setup_logger(output_root: Path) -> logging.Logger:
    ...
    logger = logging.getLogger("55_extract_embeddings_provgigapath_from_advanced_tiles")
    logger.setLevel(logging.INFO)
    logger.handlers.clear()
    formatter = logging.Formatter("%(asctime)s | %(levelname)s | %(message)s")
    file_handler = logging.FileHandler(
        logs_dir / "55_extract_embeddings_provgigapath_from_advanced_tiles.log",
        encoding="utf-8",
    )
