"""Evaluate DTFD-MIL advanced binary best checkpoint on valid and test splits."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Evalua DTFD-MIL binario advanced con threshold Youden desde valid."
    )
    parser.add_argument(
        "--config",
        type=Path,
...
def forward_batch(
    ...
    for features, label in zip(batch["features"], labels):
        ...
        loss = bag_loss + float(pseudo_bag_loss_weight) * pseudo_loss + float(instance_loss_weight) * instance_loss
...
def main() -> None:
    ...
    pseudo_weight = float(config.get("pseudo_bag_loss_weight", 0.5))
    ...
    valid_result = evaluate_split(model, valid_loader, criterion, device, pseudo_weight, instance_weight)
    test_result = evaluate_split(model, test_loader, criterion, device, pseudo_weight, instance_weight)
    ...
    print("[INFO] Evaluacion DTFD-MIL advanced completada")
