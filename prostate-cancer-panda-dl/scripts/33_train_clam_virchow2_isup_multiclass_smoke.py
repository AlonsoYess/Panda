"""Short training smoke test for CLAM + Virchow2 ISUP multiclass.

This script intentionally does not save checkpoints, plots, or official
metrics. It only verifies that a small multiclass training loop runs.
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Entrenamiento corto smoke de CLAM + Virchow2 ISUP multiclass."
    )
    parser.add_argument(
        "--embeddings-root",
        type=Path,
...
def label_distribution(labels: Iterable[int]) -> dict[int, int]:
    ...
    return {class_id: int(counts.get(class_id, 0)) for class_id in sorted(VALID_ISUP_GRADES)}
...
def train_smoke_epoch(
    *,
    ...
    for batch_index, batch in enumerate(
        tqdm(loader, desc=f"Train smoke epoch {epoch}", leave=False),
        start=1,
    ):
...
def validate_smoke(
    *,
    ...
    with torch.inference_mode():
        for batch_index, batch in enumerate(
            tqdm(loader, desc="Valid smoke", leave=False),
            start=1,
        ):
            if batch_index > int(max_batches):
                break
...
def print_checklist(status: dict[str, bool]) -> bool:
    ...
    ordered = [
        ...
        "Listo para entrenamiento completo multiclase",
    ]
    all_ok = True
    for item in ordered:
...
def run(args: argparse.Namespace) -> int:
    status = {
        ...
        "Modelo CLAMMulticlass creado": False,
        ...
        "Listo para entrenamiento completo multiclase": False,
    }
    device = resolve_device(args.device)

    print("TRAINING SMOKE: CLAM + Virchow2 ISUP multiclass")
    ...
    print("\nCargando etiquetas para distribucion y class weights...")
    train_labels = train_dataset.load_labels()
    valid_labels = valid_dataset.load_labels()
    print(f"Distribucion ISUP train: {label_distribution(train_labels)}")
    print(f"Distribucion ISUP valid: {label_distribution(valid_labels)}")
    ...
    status["Modelo CLAMMulticlass creado"] = True
    print(f"Modelo creado: CLAMMulticlass(input_dim=1280, num_classes=6)")
    ...
    status["Listo para entrenamiento completo multiclase"] = (
        finite_train
        and finite_valid
    ...
    print(
        'python scripts/33_train_clam_virchow2_isup_multiclass_smoke.py '
...
def main() -> None:
    try:
        ...
        print(f"[ERROR] Training smoke detenido: {type(exc).__name__}: {exc}")
