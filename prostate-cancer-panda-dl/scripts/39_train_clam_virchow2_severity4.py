"""Full training for CLAM + Virchow2 severity 4-class classification.

This script trains the new severity 4-class experiment only. It does not touch
the existing binary, ISUP multiclass, or validated severity smoke scripts.
"""
...
try:
    ...
    raise SystemExit(
        ...
        "severity 4-class. Instala scikit-learn antes de ejecutar este script."
    ) from exc
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Entrenamiento completo de CLAM + Virchow2 severity 4-class."
    )
...
def compute_class_weights(labels: List[int], num_classes: int = NUM_CLASSES) -> torch.Tensor:
    ...
    for class_id in range(num_classes):
        ...
        if count == 0:
            raise ValueError(f"No hay ejemplos de la clase severity {class_id} en train.")
...
def build_config(args: argparse.Namespace, paths: dict[str, Path], device: torch.device) -> Dict[str, Any]:
    return {
        "experiment_name": "clam_virchow2_severity4",
        "task": "severity_label 4-class",
...
def run(args: argparse.Namespace) -> int:
    ...
    try:
        logger.log("=== CLAM + Virchow2 severity 4-class training ===")
