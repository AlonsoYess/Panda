"""Train resumable CLAM on precomputed Virchow2 embeddings."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Entrena CLAM binario con embeddings Virchow2 1280-D."
    )
    parser.add_argument(
        "--config",
        type=Path,
        default=Path("configs/clam_virchow2_train_binary.yaml"),
    )
...
def load_config(path: Path) -> Dict[str, Any]:
    ...
    if not isinstance(config, dict):
        raise ValueError("La configuracion CLAM + Virchow2 debe ser un objeto YAML.")
    missing = sorted(REQUIRED_CONFIG_KEYS.difference(config))
    if missing:
        raise ValueError(f"Faltan claves requeridas en config CLAM + Virchow2: {missing}")
    return config
...
def apply_overrides(config: Dict[str, Any], args: argparse.Namespace) -> Dict[str, Any]:
    ...
    if args.output_root is not None:
        ...
        updated["entregables_dir"] = str(
            output_root.parent / "entregables" / "clam_virchow2_binary_resultados"
        )
    return updated
...
def validate_config(config: Dict[str, Any]) -> None:
    if int(config["input_dim"]) != EXPECTED_EMBEDDING_DIM:
        raise ValueError("input_dim debe ser 1280 para CLAM + Virchow2.")
...
def train_one_epoch_clam(
    ...
    for batch in tqdm(loader, desc="Train CLAM + Virchow2", leave=False):
    ...
    if not total_losses:
        raise RuntimeError("El DataLoader de entrenamiento CLAM + Virchow2 no produjo batches.")
    return {
...
def evaluate_clam(
    ...
    with torch.inference_mode():
        for batch in tqdm(loader, desc="Evaluate CLAM + Virchow2", leave=False):
    ...
    if not losses:
        raise RuntimeError("El DataLoader de validacion CLAM + Virchow2 no produjo batches.")
...
def run(args: argparse.Namespace) -> int:
    ...
    print(f"[INFO] Train WSI detectadas: {len(train_dataset)}")
    print(f"[INFO] Valid WSI detectadas: {len(valid_dataset)}")
    print(f"[INFO] Device: {device}")
    print(
        f"[INFO] Parametros: {total_parameters:,} total, "
        f"{trainable_parameters:,} entrenables"
    )
    ...
    if args.dry_run:
        ...
        print("[INFO] Dry-run CLAM + Virchow2 completado; no se escribieron checkpoints.")
        print(f"[INFO] Test WSI detectadas: {test_count}")
        print(f"[INFO] sample slide_id: {sample['slide_id']}")
        print(f"[INFO] features shape: {tuple(sample['features'].shape)}")
        print(f"[INFO] embedding_dim: {sample['metadata']['embedding_dim']}")
        print(f"[INFO] encoder_family: {sample['metadata']['encoder_family']}")
        ...
        print(f"[INFO] output_root esperado: {config['output_root']}")
        return 0
    ...
    if bool(config["resume"]):
        ...
        print("[INFO] Resume desactivado. Iniciando entrenamiento CLAM + Virchow2 desde cero.")
    ...
    print(f"[INFO] Entrenamiento CLAM + Virchow2 finalizado. Best epoch: {best_epoch}")
