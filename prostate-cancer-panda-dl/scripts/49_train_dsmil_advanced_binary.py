"""Train DSMIL binary MIL on advanced Virchow2 embeddings."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Entrena DSMIL binario con embeddings Virchow2 advanced."
    )
    parser.add_argument(
        "--config",
        type=Path,
...
def load_config(path: Path) -> Dict[str, Any]:
    ...
    if missing:
        raise ValueError(f"Faltan claves requeridas en config DSMIL: {missing}")
    return config
...
def validate_config(config: Dict[str, Any]) -> None:
    ...
    if encoder_name != "virchow2":
        raise ValueError("DSMIL advanced solicitado aqui solo usa encoder_name=virchow2.")
    ...
    for key in ("epochs", "batch_size_bags", "hidden_dim"):
        if int(config[key]) < 1:
...
def _dsmil_loss(
    ...
    total_loss = bag_loss + float(instance_loss_weight) * instance_loss
    return total_loss, bag_loss, instance_loss, bag_logit, max_instance_logit
...
def _forward_batch(
    ...
    max_instance_logits: List[torch.Tensor] = []
    for features, label in zip(batch["features"], labels):
        total_loss, bag_loss, instance_loss, bag_logit, max_instance_logit = _dsmil_loss(
            model=model,
            features=features,
            label=label,
            criterion=criterion,
...
def train_one_epoch_dsmil(
    ...
    for batch in tqdm(loader, desc="Train DSMIL advanced", leave=False):
        optimizer.zero_grad(set_to_none=True)
        with torch.autocast(device_type=device.type, dtype=torch.float16, enabled=amp_enabled):
            _, loss, bag_loss, instance_loss, _ = _forward_batch(
                model, batch, criterion, device, instance_loss_weight
            )
    ...
    if not total_losses:
        raise RuntimeError("El DataLoader de entrenamiento DSMIL advanced no produjo batches.")
    return {
...
def evaluate_dsmil(
    ...
    if not losses:
        raise RuntimeError("El DataLoader de validacion DSMIL advanced no produjo batches.")
...
def dry_run(config: Dict[str, Any], device: torch.device) -> None:
    ...
    print("[DRY-RUN] DSMIL advanced binary")
...
def main() -> None:
    ...
    for epoch in range(start_epoch, int(config["epochs"]) + 1):
        train_metrics = train_one_epoch_dsmil(
            model,
            train_loader,
            optimizer,
            criterion,
        ...
        save_loss_history(history, plots_dir / "training_loss.png", title_prefix="DSMIL Advanced")
        save_metric_history(history, plots_dir / "validation_auc_f1.png", title_prefix="DSMIL Advanced")
        print(
