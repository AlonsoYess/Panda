"""Final test evaluation for CLAM + Virchow2 ISUP 0-5 multiclass.

This script only evaluates the already trained multiclass model. It does not
train, overwrite checkpoints, or touch any binary experiment outputs.
"""
...
try:
    ...
    raise SystemExit(
        "[ERROR] scikit-learn es requerido para la evaluacion final multiclase. "
...
DEFAULT_ENTREGABLES_DIR = Path(
    ...
    "clam_virchow2_isup_multiclass_resultados"
)
NUM_CLASSES = 6
ISUP_LABELS = list(range(NUM_CLASSES))


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Evaluacion final test de CLAM + Virchow2 ISUP multiclass."
    )
...
def evaluate(
    *,
    ...
    with torch.inference_mode():
        for batch in tqdm(loader, desc="Evaluando test", leave=False):
...
def multiclass_metrics(labels: list[int], predictions: list[int], loss: float) -> dict[str, Any]:
    report = classification_report(
        ...
        predictions,
        labels=ISUP_LABELS,
        target_names=[f"ISUP_{label}" for label in ISUP_LABELS],
        output_dict=True,
        zero_division=0,
    )
    matrix = confusion_matrix(labels, predictions, labels=ISUP_LABELS)
    return {
...
def predictions_dataframe(results: dict[str, Any]) -> pd.DataFrame:
    ...
    for index, slide_id in enumerate(results["slide_ids"]):
        ...
        row = {
            ...
            "y_true_isup": y_true,
            "y_pred_isup": y_pred,
            "prob_isup_0": float(probabilities[0]),
            "prob_isup_1": float(probabilities[1]),
            "prob_isup_2": float(probabilities[2]),
            "prob_isup_3": float(probabilities[3]),
            "prob_isup_4": float(probabilities[4]),
            "prob_isup_5": float(probabilities[5]),
...
def print_checklist() -> None:
    print("\nCHECKLIST FINAL EVALUACION TEST ISUP")
    checks = [
        "Checkpoint best_model.pt cargado",
        "Dataset test cargado",
        "Modelo en modo eval",
        "Softmax + argmax aplicado",
        "Metricas multiclase calculadas",
        "Matriz de confusion 6x6 generada",
...
def run(args: argparse.Namespace) -> int:
    ...
    multiclass = multiclass_metrics(labels, predictions, results["loss"])
    binary = binary_derived_metrics(labels, predictions)
    multiclass_matrix = np.asarray(multiclass["confusion_matrix"], dtype=int)
    ...
    report_df = pd.DataFrame(multiclass["classification_report"]).transpose()
    ...
    multiclass_json_path = metrics_dir / "test_metrics_multiclass.json"
    multiclass_csv_path = metrics_dir / "test_metrics_multiclass.csv"
    predictions_path = metrics_dir / "test_predictions_multiclass.csv"
    isup_matrix_path = metrics_dir / "test_confusion_matrix_isup_6x6.csv"
    report_path = metrics_dir / "test_classification_report_isup.csv"
    ...
    isup_png_path = metrics_dir / "test_confusion_matrix_isup_6x6.png"
    ...
    save_json(
        ...
        multiclass_json_path,
    )
    metrics_csv_df.to_csv(multiclass_csv_path, index=False)
    predictions_df.to_csv(predictions_path, index=False)
    save_confusion_matrix_csv(
        multiclass_matrix,
        isup_matrix_path,
        labels=[f"pred_isup_{label}" for label in ISUP_LABELS],
    )
    ...
    if save_confusion_matrix_png(
        multiclass_matrix,
        isup_png_path,
        labels=[f"ISUP {label}" for label in ISUP_LABELS],
        title="CLAM + Virchow2 ISUP 0-5 - Test",
    ):
        paths_to_copy.append(isup_png_path)
    if save_confusion_matrix_png(
        binary_matrix,
        binary_png_path,
        labels=["No cancer", "Cancer"],
        title="CLAM + Virchow2 ISUP derivado binario - Test",
    ):
        paths_to_copy.append(binary_png_path)

    copy_to_entregables(paths_to_copy, entregables_dir)

    print("\n=== Evaluacion final CLAM + Virchow2 ISUP multiclass ===")
    ...
    print(f"test_loss: {multiclass['test_loss']:.6f}")
    print(f"test_accuracy: {multiclass['test_accuracy']:.6f}")
    print(f"test_macro_f1: {multiclass['test_macro_f1']:.6f}")
    print(f"test_weighted_f1: {multiclass['test_weighted_f1']:.6f}")
    print(f"test_balanced_accuracy: {multiclass['test_balanced_accuracy']:.6f}")
    print(f"test_qwk: {multiclass['test_qwk']:.6f}")
    print("\nMatriz de confusion ISUP 6x6:")
    ...
    for path in [
        ...
        print(f"- {path}")
    if isup_png_path.is_file():
        print(f"- {isup_png_path}")
...
def main() -> None:
    try:
        ...
        print(f"[ERROR] Evaluacion detenida: {type(exc).__name__}: {exc}")
