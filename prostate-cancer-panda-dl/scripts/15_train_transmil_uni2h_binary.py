"""Train resumable TransMIL on precomputed UNI2-h embeddings."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Entrena TransMIL binario con embeddings UNI2-h 1536-D."
    )
    parser.add_argument(
        "--config",
        type=Path,
        default=Path("configs/transmil_uni2h_train_binary.yaml"),
    )
...
def load_config(path: Path) -> Dict[str, Any]:
    ...
    if not isinstance(config, dict):
        raise ValueError("La configuracion TransMIL debe ser un objeto YAML.")
    missing = sorted(REQUIRED_CONFIG_KEYS.difference(config))
    if missing:
        raise ValueError(f"Faltan claves requeridas en config TransMIL: {missing}")
    return config
...
def validate_config(config: Dict[str, Any]) -> None:
    ...
    if int(config["input_dim"]) != EXPECTED_EMBEDDING_DIM:
        raise ValueError("input_dim debe ser 1536 para TransMIL + UNI2-h.")
...
def train_one_epoch_transmil(
    ...
    for batch in tqdm(loader, desc="Train TransMIL", leave=False):
...
def evaluate_transmil(
    ...
    with torch.inference_mode():
        for batch in tqdm(loader, desc="Evaluate TransMIL", leave=False):
...
def run(args: argparse.Namespace) -> int:
    ...
    if args.dry_run:
        ...
        print("[INFO] Dry-run TransMIL completado; no se escribieron checkpoints.")
    ...
    if bool(config["resume"]):
        ...
        print("[INFO] Resume desactivado. Iniciando entrenamiento TransMIL desde cero.")
    ...
    print(f"[INFO] Entrenamiento TransMIL finalizado. Best epoch: {best_epoch}")
