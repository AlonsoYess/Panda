"""Evaluate ACMIL advanced binary best checkpoint on valid and test splits."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Evalua ACMIL binario advanced con threshold Youden desde valid."
    )
    parser.add_argument(
        "--config",
        type=Path,
...
def main() -> None:
    ...
    branch_weight = float(config.get("branch_loss_weight", 0.5))
    instance_weight = float(config.get("instance_loss_weight", 0.3))
    diversity_weight = float(config.get("diversity_loss_weight", 0.05))
    ...
    valid_result = evaluate_split(
        model, valid_loader, criterion, device, branch_weight, instance_weight, diversity_weight
    )
    test_result = evaluate_split(
        model, test_loader, criterion, device, branch_weight, instance_weight, diversity_weight
    )
    ...
    print("[INFO] Evaluacion ACMIL advanced completada")
