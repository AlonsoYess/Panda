"""Final validation and test evaluation for TransMIL + UNI2-h."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Evalua best_model.pt de TransMIL + UNI2-h en valid y test."
    )
    parser.add_argument(
        "--config",
        type=Path,
...
def run(args: argparse.Namespace) -> int:
    ...
    save_confusion_matrix(
        ...
        f"TransMIL Valid Confusion Matrix (Youden={threshold_youden:.4f})",
    )
    save_confusion_matrix(
        test_youden_metrics["confusion_matrix"],
        plots_dir / "confusion_matrix_test.png",
        f"TransMIL Test Confusion Matrix (Valid Youden={threshold_youden:.4f})",
    )
    save_roc_curve(
        valid_result["labels"],
        valid_result["probabilities"],
        plots_dir / "roc_valid.png",
        "TransMIL + UNI2-h Validation ROC",
    )
    save_roc_curve(
        test_result["labels"],
        test_result["probabilities"],
        plots_dir / "roc_test.png",
        "TransMIL + UNI2-h Test ROC",
    )
