"""Dataset utilities for Virchow2 severity 4-class MIL bags."""
...
class Virchow2Severity4DatasetError(ValueError):
    """Raised when a Virchow2 bag is invalid for severity 4-class learning."""


def isup_to_severity4(isup_grade: int) -> int:
    """Map ISUP grade 0-5 to severity labels 0-3."""
    ...
    if grade in (4, 5):
        return 3
    raise Virchow2Severity4DatasetError(f"isup_grade fuera de rango 0-5: {isup_grade}")
...
def _as_int(value: Any, *, field_name: str, path: Path) -> int:
    try:
        ...
        raise Virchow2Severity4DatasetError(
            f"{field_name} no es convertible a int en: {path}"
        ) from exc


def load_virchow2_severity4_payload(
    path: Path,
    *,
    expected_dim: int = EXPECTED_EMBEDDING_DIM,
    expected_split: str | None = None,
) -> Dict[str, Any]:
    """Load one Virchow2 .pt artifact and validate severity-label requirements."""
    ...
    for key in ("slide_id", "features", "isup_grade", "split"):
        if key not in payload:
            raise Virchow2Severity4DatasetError(f"Falta {key} en: {artifact_path}")
    ...
    if not isinstance(features, torch.Tensor):
        raise Virchow2Severity4DatasetError(f"features no es Tensor en: {artifact_path}")
    if features.ndim != 2:
        raise Virchow2Severity4DatasetError(
    ...
    if int(features.shape[0]) < 1:
        raise Virchow2Severity4DatasetError(f"La bolsa no contiene tiles en: {artifact_path}")
    ...
    severity_label = isup_to_severity4(isup_grade)
    if severity_label not in VALID_SEVERITY_LABELS:
        raise Virchow2Severity4DatasetError(
            f"severity_label debe estar en 0-3 en {artifact_path}; recibido {severity_label}."
        )
...
class Virchow2Severity4Dataset(Dataset):
    """Lazy per-WSI bag dataset using mapped severity_label as target."""
    ...
    def __init__(
        ...
        self.isup_grades: List[int] = []
        self.slide_ids: List[str] = []
        if validate_on_init:
            self.load_labels()

    def load_labels(self) -> List[int]:
        """Validate artifacts and cache severity labels for class-weight inspection."""
        self.labels = []
        self.isup_grades = []
        self.slide_ids = []
        for path in tqdm(
            self.files,
            desc=f"Validando Virchow2 severity4/{self.split}",
            leave=False,
        ):
            payload = load_virchow2_severity4_payload(
                path,
                expected_dim=self.expected_dim,
                expected_split=self.split,
            )
            self.labels.append(int(payload["severity_label"]))
            self.isup_grades.append(int(payload["isup_grade"]))
    ...
    def __getitem__(self, index: int) -> Dict[str, Any]:
        ...
        severity_label = int(payload["severity_label"])
        isup_grade = int(payload["isup_grade"])
        label = torch.tensor(severity_label, dtype=torch.long)
        metadata = {
...
def virchow2_severity4_bag_collate_fn(batch: List[Dict[str, Any]]) -> Dict[str, Any]:
    """Collate variable-size Virchow2 severity bags without tile padding."""
    return {
        "features": [item["features"] for item in batch],
        "labels": torch.stack([item["label"] for item in batch]).long(),
        "severity_labels": torch.stack([item["severity_label"] for item in batch]).long(),
        "isup_grades": torch.stack([item["isup_grade"] for item in batch]).long(),
