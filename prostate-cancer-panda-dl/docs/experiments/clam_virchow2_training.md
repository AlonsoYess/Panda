# CLAM + Virchow2 para deteccion binaria

## Objetivo

Este experimento entrena **CLAM + Virchow2** para clasificacion binaria de cancer
de prostata usando embeddings Virchow2 previamente generados.
...
- `modelo/`
- `regiones_relevantes/`
- `resumen_resultados_clam_virchow2.json`

## Relacion con experimentos previos

Este flujo reutiliza la arquitectura CLAM ya validada en CLAM + UNI2-h, pero
cambia la entrada de `1536` a embeddings Virchow2 de `1280` dimensiones.
...
```bash
python scripts/23_train_clam_virchow2_binary.py \
  --config configs/clam_virchow2_train_binary.yaml \
  --dry-run \
  --device cuda
```
...
python scripts/23_train_clam_virchow2_binary.py \
  ...
  --max-valid-slides 32 \
  --device cuda \
  --output-root /content/drive/MyDrive/PANDA_PROSTATE/outputs/clam_virchow2_binary_smoke \
  --no-resume
```
...
```bash
python scripts/24_evaluate_clam_virchow2_binary.py \
  --config configs/clam_virchow2_train_binary.yaml \
  --device cuda \
  --output-root /content/drive/MyDrive/PANDA_PROSTATE/outputs/clam_virchow2_binary_smoke \
  --max-valid 32 \
...
`test_attention_scores.csv` registra atencion CLAM por tile para apoyar la
revision de regiones relevantes. Esta fase no genera paneles visuales de tiles.
